...
exports.versions_folder = __dirname + "/../chainData";
async function getLatestInfo() {
    ...
    return Object.assign(map, {
        getAllChainData() {
            let result = { checkPointData: {}, exeProgram: {}, exeProgramWithCheckPointData: {} };
            const keys = map.keys();
            for (let type of keys) {
                ...
                else if (type === "exeProgramWithCheckPointData") {
                    result.exeProgramWithCheckPointData = map.get(type);
                }
            }
            return result;
        },
        getChainDataByType(type) {
            if (!map.get(type)) {
